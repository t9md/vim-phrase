...
let s:phrase_separator = repeat('=', 70)
...
function! s:ft_info.init()
  ...
  let self._table['mason'] = { 'ext': 'mason', 'cmt': ['<% #', '%>'] }
endfunction
call s:ft_info.init()
...
function! phrase#create(...)"{{{
  let body = a:0 > 0
        \ ? getline(a:1, a:2)
        \ : s:selected_lines()
  call phrase#start('create', {'body': body } )
endfunction"}}}

function! phrase#edit(...)"{{{
  let ext = a:0 > 0 ? a:1 : ''
  call phrase#start('edit', {"ext": ext })
endfunction"}}}
...
function! phrase#start(cmd, opt) "{{{
  call s:ensure_phrase_dir()
  let author = g:phrase_author

  let ext = get(a:opt, 'ext', '')
  let ext = !empty(ext)
        \ ? ext
        \ : exists('b:phrase_ext') && !empty('b:phrase_ext')
        \ ? b:phrase_ext
  ...
  elseif a:cmd == 'create'
    let title = inputdialog("Phrase[ " . ext . " ]","", -1)
    ...
    execute "normal! 3ggV".(len(a:opt['body'])-1)."jo"
  endif
endfunction"}}}
